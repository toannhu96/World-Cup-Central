...
const BASE_URL = ""; // Relative to the same domain in Zalo Mini App
...
export async function fetchScoreboard(): Promise<ScoreboardResponse> {
  try {
    ...
    console.warn("Using local scoreboard fallback:", error);
    return {
      source: "local",
      events: [],
      hasLive: false,
...
export async function fetchGroups(): Promise<GroupsResponse> {
  try {
    ...
    console.warn("Using local groups fallback:", error);
    return {
      source: "local",
      groups: [],
      teamGroup: {},
...
export async function fetchMatchSummary(espnId: string): Promise<MatchSummary | null> {
  try {
    const res = await fetch(`${BASE_URL}/api/football/summary?event=${espnId}`);
    if (!res.ok) throw new Error(`Summary fetch failed: ${res.status}`);
    const data = await res.json();
    
    // Parse the ESPN summary data into our MatchSummary format
    // This assumes the API proxy handles the transformation or we do it here.
    // Given the previous pattern, we expect a clean object or we map it.
